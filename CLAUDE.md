...
- **Saque/Depósito** (`TransferModal.tsx`): botão "Saque / Depósito" dentro de cada dia expandido (não dentro do editor de saldo por casa — pedido explícito do usuário, "esse botão n precisa ficar dentro de onde cadastra o valor da casa"), visível só pra `day.date <= hoje`. O modal tem um toggle Saque/Depósito (troca o label do `<select>` entre "De qual casa"/"Pra qual casa" e o texto explicativo), um `<select>` com as casas do dia **exceto** qualquer uma chamada "Banco" (filtrado no `BetMonthDetailPage` antes de passar a prop, case-insensitive) e um campo de valor — `POST /bets/months/{monthId}/houses/{houseId}/transfer`. Erro (ex: usuário sem casa "Banco" cadastrada) aparece via `extractErrorMessage`, mesmo padrão dos outros modais.
