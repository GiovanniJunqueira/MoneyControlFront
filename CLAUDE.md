...
- **Painel em funil** (pedido explícito do usuário, "como a Apple faria"): `BetsHomePage` mostra só o lucro total (`GET /bets/overview`, em unidades usando o valor de unidade mais atual — não soma unidade por mês) + a lista de todos os meses (`GET /bets/months`, cada um já com seu próprio lucro em R$/unidades e uma badge "Atual" se for o mês aberto). Clicar num mês **navega** pra `BetMonthDetailPage` (`/bets/months/:monthId`) — decisão do usuário, prefere tela própria a expandir inline (fica pesado com 30 dias). Dentro dela, cada dia é uma linha que expande **em sanfona** (accordion, `Set<string>` local de datas expandidas) mostrando o resultado de cada casa naquele dia; o dia de hoje já vem expandido quando o mês é o aberto. Editar o saldo de uma casa num dia só aparece se o mês ainda está aberto (mês fechado é histórico read-only).
- Modais: `CreateBetHouseModal`/`ManageBetHousesModal` (cópia do padrão `CreateTabModal`/`ManageTabsModal`, mas essa busca as casas sozinha via `GET /bets/houses` em vez de receber por prop, porque também cuida da reordenação), `UpdateBetBalanceModal` (props explícitas `houseId/houseName/date/currentBalance/startOfDayBalance` em vez de um objeto de tipo — o `date` viaja no POST pra permitir editar qualquer dia do mês aberto, não só hoje), `StartBetMonthModal`, `ChangeUnitValueModal` (mostrado dentro de `BetMonthDetailPage` agora, não mais na Home).
