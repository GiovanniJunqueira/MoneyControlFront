...
- **Card "Geral do mês, por casa"** (acima da lista de dias): soma do resultado de cada casa ao longo do mês inteiro (`data.houseSummaries`, já vem pronto do backend — a página só formata). Pode não bater com o total do mês lá em cima se teve algum ajuste de saldo inicial (depósito/saque) no meio — é esperado, não é bug (ver `BetService`).
