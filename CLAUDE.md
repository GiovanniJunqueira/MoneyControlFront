...
- **Card "Geral do mês, por casa"** (acima da lista de dias): soma do resultado de cada casa ao longo do mês inteiro (`data.houseSummaries`, já vem pronto do backend — a página só formata). `data.profitLoss`/`profitLossUnits` no topo do mês **é** a soma dessas casas (mesma fonte no backend) — os dois números sempre batem, incluindo quando tem dia negativo. Também é **recolhível, começando fechado** (`geralOpen` local, mesmo padrão accordion dos dias) — o card em si sempre aparece (se tiver alguma casa), só a lista de casas dentro é que fica escondida até clicar.
