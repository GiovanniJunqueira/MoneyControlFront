...
- Modais: `CreateBetHouseModal`/`ManageBetHousesModal` (cópia do padrão `CreateTabModal`/`ManageTabsModal`, mas essa busca as casas sozinha via `GET /bets/houses` em vez de receber por prop, porque também cuida da reordenação), `UpdateBetBalanceModal` (props explícitas `monthId/houseId/houseName/date/currentBalance/startOfDayBalance`; a rota de salvar leva `monthId` — `POST /bets/months/{monthId}/houses/{houseId}/balance`, não infere mais "o mês aberto"). Tem DOIS campos — "saldo inicial do dia" e "saldo final do dia" — pra permitir registrar um depósito/saque na casa sem contar como resultado de aposta. **Não usa mais um booleano de "touched"** pra decidir se manda o ajuste do saldo inicial (era frágil, causava o campo "perder" o valor sem motivo claro) — em vez disso, compara o valor **submetido** com o valor que veio como padrão (`startOfDayBalance`, prop) e só manda como ajuste se for realmente diferente; se bater, manda `null` (mantém automático). `StartBetMonthModal`, `ChangeUnitValueModal` (mostrado dentro de `BetMonthDetailPage`, não mais na Home).
